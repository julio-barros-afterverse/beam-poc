...
public class MinimalWordCount {
    ...
    public static void main(String[] args) {
        ...
        p.apply(TextIO.read().from("/Users/julio.barros/data/textao"))
            ...
            .apply(TextIO.write().to("/Users/julio.barros/data/wordcounts"));

        p.run().waitUntilFinish();

        System.out.println("Pipeline 1 (WordCount) finished");

        Pipeline p2 = Pipeline.create(options);

        p2.apply(PubsubIO.readStrings().fromSubscription("projects/playkids-infra-974/subscriptions/local-beam"))

            .apply(
                MapElements.into(TypeDescriptor.of(Serialized.class))
                    .via(Serialized::new)
            )
            ...
            .apply(
                JdbcIO.<String>write()
                    .withPreparedStatementSetter(
                        (input, ps) -> ps.setString(1, input)
                    )
                    .withStatement("insert into test_beam values (?)")
                    .withDataSourceConfiguration(JdbcIO.DataSourceConfiguration.create(
                        "org.postgresql.Driver",
                        "jdbc:postgresql://localhost:5432/bojack"
                    ))
            );

        p2.run().waitUntilFinish();
        System.out.println("Pipeline 2 (PubSub) finished");
    }
